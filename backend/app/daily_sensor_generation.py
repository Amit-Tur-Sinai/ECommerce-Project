...
def generate_initial_compliance_data(db: Session, business: Business):
    ...
    for event in climate_events:
        for rec_text, risk_level in recs_config.get(event, []):
            # Deterministic per-business: use hash of business_id + rec_index to decide status
            # This gives each business a different mix of implemented/pending
            status_hash = int(hashlib.sha256(
                f"rec-status-{business.business_id}-{rec_index}".encode()
            ).hexdigest(), 16) % 100
            
            # ~70% chance implemented, ~30% pending (varies per business)
            status = (
