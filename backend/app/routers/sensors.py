...
async def get_recommendations(
    ...
    """Get all recommendation tracking items for the current user's business."""
    if not current_user.business_id:
        raise HTTPException(status_code=404, detail="Business not found")

    recommendations = db.query(RecommendationTracking).filter(
        RecommendationTracking.business_id == current_user.business_id
    ).order_by(RecommendationTracking.status, RecommendationTracking.climate_event).all()

    return {
        "recommendations": [
            {
                "tracking_id": r.tracking_id,
                "climate_event": r.climate_event,
